...
for ip, rasn, nic in peers:
    ...
    if role not in ("core1", "core2"):
        out.append(f"  neighbor {ip} route-map AS3-PREFER in")
...
if role in ("edge", "access"):
    out += [
        "!",
        "bgp as-path access-list AS3-PATH permit _3_",
        "!",
        "route-map AS3-PREFER permit 10",
        " match as-path AS3-PATH",
        " set local-preference 150",
        "route-map AS3-PREFER permit 20",
        "exit",
        "!",
    ]
...
# #ASN 1 will deny all non bgp routes in the following example, this is to put full preference on low latency route for classic and l4s
